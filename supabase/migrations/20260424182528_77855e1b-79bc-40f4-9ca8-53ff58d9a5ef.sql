...
BEGIN
  ...
  LOOP
    UPDATE public.cms_arcanos SET
      ...
      keywords          = COALESCE(
                            ARRAY(SELECT jsonb_array_elements_text(_item->'keywords')),
                            keywords
                          ),
      tags              = COALESCE(
                            ARRAY(SELECT jsonb_array_elements_text(_item->'tags')),
                            tags
                          ),
    ...
    WHERE type = (_item->>'type')::arcano_type
      ...
      AND (
        (_item->>'naipe' IS NULL AND naipe IS NULL)
        OR (naipe = (_item->>'naipe')::arcano_naipe)
      );
...
GRANT EXECUTE ON FUNCTION public.apply_arcano_backfill(jsonb) TO authenticated;
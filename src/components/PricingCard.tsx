...
export const PricingCard = ({ plan }: { plan: Plan }) => {
  ...
  return (
    <div className={`relative glass-strong p-8 flex flex-col h-full ${plan.highlight ? "neon-shadow border-accent/40" : ""}`}>
      {plan.priceNote && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[11px] font-semibold tracking-wider bg-gradient-to-r from-primary to-accent text-white uppercase whitespace-nowrap">
          {plan.priceNote}
        </div>
      )}
      <h3 className="text-xl font-heading font-bold">{plan.name}</h3>
      <p className="text-muted-foreground text-[15px] leading-relaxed mt-2 min-h-[48px]">{plan.description}</p>
